 ...
 * outside os.tmpdir(). See mcp/daleel-mcp.js.
 */
...
const MCP = path.join(__dirname, '..', 'mcp', 'daleel-mcp.js');
...
function driveCheckCode(ext, code) {
  return new Promise((resolve) => {
    ...
    send({ jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'daleel_check_code', arguments: { code, ext } } });
    setTimeout(done, 5000); // safety net
  });
}

test('check_code rejects a path-traversal ext (no arbitrary write/delete)', async () => {
  const victimDir = fs.mkdtempSync(path.join(os.tmpdir(), 'daleel-victim-'));
  const victim = path.join(victimDir, 'KEEP.txt');
  fs.writeFileSync(victim, 'PRESERVE-ME');
  // Resolves via path.join(os.tmpdir(), `daleel-<ts>-<rand>` + ext) back to the victim.
