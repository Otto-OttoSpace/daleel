...
const MSG = {
  ...
  htmlLangInvalid: 'invalid lang value — use a BCP-47 code like "ar" or "en-US", not an empty or free-text value like "arabic" (WCAG 2.2 · 3.1.1)',
  nameMissing: 'icon-only control has no accessible name — add aria-label to the <button>/<a>, or a <title> inside the <svg> (WCAG 2.2 · 4.1.2)',
...
// Pull a parseable colour out of a `background` shorthand (or a bare colour):
// the LAST colour-like token, so `background: url(x) center/cover #222` → #222.
// Returns null when no literal colour is present (gradients/vars are skipped).
function bgColor(value) {
  const toks = String(value).match(/#[0-9a-fA-F]{3,8}|rgba?\([^)]*\)|hsla?\([^)]*\)|[a-zA-Z][a-zA-Z-]*/g) || [];
  for (let i = toks.length - 1; i >= 0; i--) if (parseColor(toks[i])) return toks[i];
  return null;
}
// color + background(-color | shorthand) in one declaration container (a rule, or
// an inline style="" declaration list) → contrast. `each` = direct children only,
// so a root check sees inline decls and a rule check sees just that rule.
function containerContrast(container, baseLine, findings) {
  let fg = null, bg = null, cline = baseLine;
  container.each(d => {
    if (d.type !== 'decl') return;
    const p = d.prop.toLowerCase();
    if (p === 'color') { fg = d.value; cline = baseLine + ((d.source && d.source.start ? d.source.start.line : 1) - 1); }
    else if (p === 'background-color') bg = d.value;
    else if (p === 'background') { const c = bgColor(d.value); if (c) bg = c; }
  });
  if (fg && bg) checkContrast(fg, bg, cline, findings);
}
...
// A literal <html lang="…"> value, captured for BCP-47 validation.
const HTML_LANG_VAL_RE = /<html\b[^>]*?(?<![-\w])lang\s*=\s*["']([^"']*)["'][^>]*>/gi;
// A conservative BCP-47 shape: a 2–3 letter primary subtag then hyphen-joined
// subtags (so ar, en-US, zh-Hans, ar-SA pass), plus the x-/i- private-use and
// grandfathered forms. Rejects "", "arabic", "english", a lone "e", "123", and
// underscore-joined "ar_SA" (invalid in HTML lang) — all genuine 3.1.1 failures.
const BCP47_RE = /^[a-zA-Z]{2,3}(?:-[a-zA-Z0-9]{1,8})*$/;
const isValidLang = v => BCP47_RE.test(v) || /^[xi]-[a-zA-Z0-9]{1,8}(?:-[a-zA-Z0-9]{1,8})*$/i.test(v);
const CLASS_ATTR_RE = /\b(?:class|className)\s*=\s*(["'])((?:(?!\1).)*)\1/g;

// Is this JSX element a direct branch of a conditional — a ternary consequent/
// alternate (`cond ? <h1/> : <h3/>`) or the right side of `&&`/`||`
// (`cond && <h3/>`)? Such an element renders conditionally, so it must not be
// compared in document order against a heading in the sibling branch.
function inConditionalBranch(openingPath) {
  const el = openingPath.parentPath; // the JSXElement
  const par = el && el.parentPath;
  if (!par || !par.node) return false;
  const pn = par.node, en = el.node;
  if (par.isConditionalExpression && par.isConditionalExpression() && (pn.consequent === en || pn.alternate === en)) return true;
  if (par.isLogicalExpression && par.isLogicalExpression() && pn.right === en) return true;
  return false;
}

// Do this JSX child list carry (or possibly carry) an accessible name for an
// interactive ancestor? Conservative by design — we only ever want to flag the
// unambiguous icon-only case, so anything that *might* be a name suppresses it:
//   text          real text content (a name)
//   expr          a dynamic {expression} child (could be text)
//   img           an <img> child (its own alt rule covers naming)
//   namedSvg      an <svg> that names itself (aria-label/labelledby/title/role
//                 attr, or a <title> child)
//   component     a capitalized component child (opaque internals — may label
//                 itself; we never flag through one)
//   bareSvg       an <svg> with none of the above — the icon we'd flag on
function jsxChildNameSignals(children, depth) {
  depth = depth || 0;
  const sig = { text: false, expr: false, img: false, namedSvg: false, component: false, bareSvg: false };
  if (depth > 6) { sig.expr = true; return sig; } // too deep to reason → assume named
  for (const c of children || []) {
    if (!c) continue;
    if (c.type === 'JSXText') { if (c.value && c.value.trim()) sig.text = true; continue; }
    if (c.type === 'JSXExpressionContainer') {
      const e = c.expression;
      if (e && e.type === 'StringLiteral') { if (e.value.trim()) sig.text = true; }
      else if (!e || e.type === 'JSXEmptyExpression') { /* {/* comment *​/} — nothing */ }
      else sig.expr = true;
      continue;
    }
    if (c.type !== 'JSXElement') continue;
    const on = c.openingElement;
    const cn = on && on.name && on.name.type === 'JSXIdentifier' ? on.name.name : '';
    const cattrs = (on && on.attributes) || [];
    const hasAttr = nm => cattrs.some(a => a.type === 'JSXAttribute' && a.name && a.name.name === nm);
    const lc = cn.toLowerCase();
    if (lc === 'img') { sig.img = true; continue; }
    if (lc === 'svg') {
      const titleChild = (c.children || []).some(gc => gc.type === 'JSXElement' && gc.openingElement && String((gc.openingElement.name || {}).name || '').toLowerCase() === 'title');
      if (hasAttr('aria-label') || hasAttr('aria-labelledby') || hasAttr('title') || hasAttr('role') || titleChild) sig.namedSvg = true;
      else sig.bareSvg = true;
      continue;
    }
    if (/^[A-Z]/.test(cn)) { sig.component = true; continue; } // opaque component
    if (hasAttr('aria-label') || hasAttr('aria-labelledby')) { sig.text = true; continue; }
    const sub = jsxChildNameSignals(c.children, depth + 1);
    for (const k of Object.keys(sig)) sig[k] = sig[k] || sub[k];
  }
  return sig;
}
...
function scanJsAst(src, ext, findings, approved) {
  ...
  babelTraverse(ast, {
    ...
    JSXOpeningElement(p) {
      ...
      const hasName = attrNames.has('aria-label') || attrNames.has('aria-labelledby') || attrNames.has('title');
      // <input type="image"> is an image submit button — spec-requires alt/name.
      if ((name === 'input' || name === 'Input') && String(attrVal('type') || '').toLowerCase() === 'image'
        && !hasSpread && !attrNames.has('alt') && !hasName)
        push('A11Y', 'a11y-img-alt', line, '<input type="image">', MSG.imgAlt);
      // role="img" makes an element a single image → its name comes from an
      // aria-label/labelledby/title, not children (svg is excluded — it can carry
      // an accessible name via a <title> child this static pass can't resolve).
      if (role === 'img' && name.toLowerCase() !== 'svg' && !ariaHidden && !hasName)
        push('A11Y', 'a11y-img-alt', line, '<' + name + ' role="img">', MSG.imgAlt);
      // Icon-only <button> / <a href> with no accessible name (WCAG 4.1.2). We
      // only flag a bare native <svg> icon — a labelled svg, any text/expression
      // child, an <img>, or an opaque component child all suppress it, and we
      // never flag through a spread (which may carry aria-label).
      if (!hasSpread && (name === 'button' || (name === 'a' && attrNames.has('href')))
        && !attrNames.has('aria-label') && !attrNames.has('aria-labelledby') && !attrNames.has('title') && !p.node.selfClosing) {
        const kids = (p.parentPath && p.parentPath.node && p.parentPath.node.children) || [];
        const sig = jsxChildNameSignals(kids);
        if (sig.bareSvg && !sig.text && !sig.expr && !sig.img && !sig.namedSvg && !sig.component)
          push('A11Y', 'a11y-name-missing', line, '<' + name + '>', MSG.nameMissing);
      }
      if ((name === 'html' || name === 'Html') && !hasSpread) {
        if (!attrNames.has('lang')) push('A11Y', 'a11y-html-lang', line, '<' + name + '>', MSG.htmlLang);
        else {
          // Validate only a literal value; `lang={locale}` is a runtime value we
          // can't (and must not) judge statically.
          const lv = attrVal('lang');
          if (typeof lv === 'string' && !isValidLang(lv.trim()))
            push('A11Y', 'a11y-html-lang-invalid', line, '<' + name + ' lang="' + lv + '">', MSG.htmlLangInvalid);
        }
      }
      ...
      if (hm && !inConditionalBranch(p)) {
        // A heading rendered as a ternary/`&&` branch is conditional — comparing
        // it against a heading in document order (which may be in the OTHER,
        // mutually-exclusive branch) is a false positive. e.g. `cond ? <h1/> :
        // <h3/>` renders exactly one, never an h1→h3 skip. Such headings neither
        // flag nor advance lastHeading.
...
function scanCss(src, findings, approved) {
  ...
  // Same-rule colour + background(-color|shorthand) → contrast (1.4.3).
  root.walkRules(rule => containerContrast(rule, 1, findings));
  return true;
}
...
function scanStyleText(css, baseLine, findings, approved) {
  ...
  // Contrast — color + background(-color|shorthand) in the SAME rule. A <style>
  // block holds many rules; previously only the FIRST color/background pair in the
  // whole block was checked (a plain regex .exec), badly under-scanning multi-rule
  // blocks. An inline style="" is a bare declaration list (root decls). (B5)
  if (postcss) {
    let root; try { root = postcss.parse(css); } catch { root = null; }
    if (root) {
      containerContrast(root, baseLine, findings);       // inline style="" decls
      root.walkRules(r => containerContrast(r, baseLine, findings)); // each <style> rule
    }
  }
}
...
function scanMarkup(src, findings, approved) {
  ...
  HTML_LANG_VAL_RE.lastIndex = 0;
  while ((m = HTML_LANG_VAL_RE.exec(masked))) { if (/\{[^}]*\}/.test(m[0])) continue; const lv = m[1].trim(); if (!isValidLang(lv)) findings.push({ cat: 'A11Y', rule: 'a11y-html-lang-invalid', line: lineAt(masked, m.index), from: '<html lang="' + m[1] + '">', msg: MSG.htmlLangInvalid }); }
